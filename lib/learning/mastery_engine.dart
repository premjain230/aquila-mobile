...
Map<String, double> calculateMastery(List<Map<String, dynamic>> evidence) {
  if (evidence.isEmpty) return {'masteryScore': 0.5, 'evidenceStrength': 0.0, 'weightedCorrectness': 0.5, 'effectiveWeight': 0.0};
  ...
  if (sumW == 0) return {'masteryScore': 0.5, 'evidenceStrength': 0.0, 'weightedCorrectness': 0.5, 'effectiveWeight': 0.0};
  final wc = sumWC / sumW;
  double adj = 0;
  if (confCount > 0) adj = (confSum / confCount - 0.5) * 0.10;
  final raw = (wc + adj).clamp(0, 1).toDouble();
  final sampleFactor = (sumW / 3).clamp(0, 1).toDouble();
  final masteryScore = (masteryPrior * (1 - sampleFactor) + raw * sampleFactor).clamp(0, 1).toDouble();
  final evidenceStrength = (sumW / 5).clamp(0, 1).toDouble();
